...
def add():
    symbols = []
    while True:
        symbol = input("Enter a stock symbol to track (or leave blank to finish): ")
        if not symbol:
            break
        symbols.append(symbol.upper())

    if not symbols:
        print("No symbols entered. Watchlist not saved.")
        return

    while True:
        name = input("Enter a name for the watchlist: ")
        if symbol(name, symbols):
            break
